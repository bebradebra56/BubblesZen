...
class BubblesZenVi(
    private val bubblesZenContext: Context,
    private val bubblesZenCallback: BubblesZenCallBack,
    private val bubblesZenWindow: Window
) : WebView(bubblesZenContext) {
    private var bubblesZenFileChooserHandler: ((ValueCallback<Array<Uri>>?) -> Unit)? = null
    fun bubblesZenSetFileChooserHandler(handler: (ValueCallback<Array<Uri>>?) -> Unit) {
        this.bubblesZenFileChooserHandler = handler
    }
    ...
    init {
        ...
        webSettings.apply {
            ...
            userAgentString =
                WebSettings.getDefaultUserAgent(bubblesZenContext).replace("; wv)", "")
                    .replace("Version/4.0 ", "")
        ...
        super.setWebViewClient(object : WebViewClient() {
            override fun shouldOverrideUrlLoading(
                ...
                return if (request?.isRedirect == true) {
                    ...
                    try {
                        bubblesZenContext.startActivity(intent)
                    } catch (e: Exception) {
                        Toast.makeText(
                            bubblesZenContext,
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                if (url?.contains("ninecasino") == true) {
                    BubblesZenApplication.bubblesZenInputMode =
                        WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN
                    Log.d(
                        BubblesZenApplication.BUBBLES_ZEN_MAIN_TAG,
                        "onPageFinished : ${BubblesZenApplication.bubblesZenInputMode}"
                    )
                    bubblesZenWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN);
                    bubblesZenEnableStableInputScroll()
                } else {
                    BubblesZenApplication.bubblesZenInputMode =
                    ...
                    bubblesZenWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
                }
            }
        ...
        super.setWebChromeClient(object : WebChromeClient() {
            ...
            override fun onShowFileChooser(
                ...
                bubblesZenFileChooserHandler?.invoke(filePathCallback)
                return true
            }
            ...
            override fun onCreateWindow(
                ...
                bubblesZenHandleCreateWebWindowRequest(resultMsg)
                return true
            }
        })
    }
    ...
    private fun bubblesZenEnableStableInputScroll() {
        evaluateJavascript(
            """
        (function() {
    ...
    fun bubblesZenFLoad(link: String) {
        super.loadUrl(link)
    }

    private fun bubblesZenHandleCreateWebWindowRequest(resultMsg: Message?) {
        if (resultMsg == null) return
        if (resultMsg.obj != null && resultMsg.obj is WebView.WebViewTransport) {
            val transport = resultMsg.obj as WebView.WebViewTransport
            val windowWebView =
                BubblesZenVi(bubblesZenContext, bubblesZenCallback, bubblesZenWindow)
            transport.webView = windowWebView
            resultMsg.sendToTarget()
            bubblesZenCallback.bubblesZenHandleCreateWebWindowRequest(windowWebView)
        }
    }
